...
void sieve(int pleft[2]) { // pleft 是来自该 stage 左端进程的输入管道
	...
	if(fork() == 0) {
		// 子进程 （下一个 stage）
		close(pright[1]); // 子进程只需要对输入管道 pright 进行读，而不需要写，降低进程打开的文件描述符数量
...
int main(int argc, char **argv) {
	...
	if(fork() == 0) {
		...
		sieve(input_pipe);
		exit(0);//为了防止子进程“逃逸”并错误地执行了属于父进程的代码。
	} else {
		// 主进程
	...
	exit(0);
	//但在 C 语言系统编程中，大家习惯于把**“主体业务分支”（写数据）和“生命周期收尾”（等待回收和退出）**在视觉上分开。 放在 else内完全可以
}
