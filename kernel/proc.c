...
procinit(void)
{
  ...
  for(p = proc; p < &proc[NPROC]; p++) {
      ...
      // p->kstack = va;
      //上述注释的代码是为所有进程进行预分配内核栈的代码（共享空间中）
  }
  kvminithart();
}
...
found:
  ...
  //创建进程的时候 为进程创建独立的内核页表 然后将专属的内核栈固定到内核页表的固定位置 建立映射
  ////// 新加部分 start //////
  ...
  if(pa == 0)
    panic("kalloc");
  uint64 va = KSTACK((int)0); // 将内核栈映射到固定的虚拟地址上
...
//我的理解是我们创建的独立内核页表的资源不能直接释放，其他进程也在用，虽然是独立页表，里面的资源是公用的
...
int
growproc(int n)//growproc() 函数也就是处理 sbrk 系统调用、负责增加进程内存的地方。
{
  uint sz;
  ...
  sz = p->sz;
  if(n > 0){
    // if(sz + n >= PLIC) {// 补上这条防线：防止用户的内存扩张撞到 PLIC 导致 remap 
    //   return -1;
    // }
    uint64 newsz;
    ...
    if(kvmcopymappings(p->pagetable, p->kernelpgtbl, sz, n) != 0) {
      uvmdealloc(p->pagetable, newsz, sz);  //回滚
      return -1;
    }
    sz = newsz;
  } else if(n < 0){
...
kvmcopymappings(np->pagetable,np->kernelpgtbl,0,p->sz)<0){//调用 kvmcopymappings，将**新进程**用户页表映射拷贝一份到新进程内核页表中
    freeproc(np);
    release(&np->lock);
    return -1;
  }
  ...
  for(;;){
    ...
    for(p = proc; p < &proc[NPROC]; p++) {
      ...
      if(p->state == RUNNABLE) {
        ...
        // 切换回全局内核页表
        kvminithart();
        //调度器/中断处理应该依赖“统一、稳定”的全局内核映射；
        // 否则会把“当前 CPU 的内核执行”错误绑定到某个进程地址空间。
