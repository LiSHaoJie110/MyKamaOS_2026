...
syscall(void)
{
  ...
  struct proc *p = myproc();
  //获取系统调用号
  num = p->trapframe->a7;// a7 寄存器里存着系统调用的编号
  // 如果系统调用编号有效(大于0且小于 syscal1s数组的长度，并且对应的处理函数存在)
  if(num > 0 && num < NELEM(syscalls) && syscalls[num]) {
    //调用对应的处理函数，并将返回值存储在 a0寄存器中
    p->trapframe->a0 = syscalls[num]();

    // 使用位运算：将 1 左移 num 位，看看 mask 对应的这一位是不是 1
    if((p->kama_syscall_trace)>>num & 1){
      // 如果匹配上了，打印：PID: syscall名字 -> 返回值
